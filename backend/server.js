...
/**
 * Resolve consumerId from either a direct ID or customer_email
 */
async function resolveConsumerId({ consumerId, customer_email }) {
  if (consumerId) {
    const result = await query('SELECT id FROM consumers WHERE id = $1', [consumerId]);
    return result.rows[0]?.id ?? null;
  }
  if (customer_email) {
    const result = await query('SELECT id FROM consumers WHERE email = $1', [customer_email]);
    return result.rows[0]?.id ?? null;
  }
  return null;
}
...
/**
 * Resolve medicine ID from a medicine name (case-insensitive fuzzy match)
 */
async function resolveMedicineId(name) {
  if (!name) return null;

  // Try exact match first
  const exact = await query(
    'SELECT id FROM medicines WHERE LOWER(medicine_name) = LOWER($1)',
    [name.trim()]
  );
  if (exact.rows.length > 0) return exact.rows[0].id;

  // Try partial match (medicine name contains the search term)
  const partial = await query(
    'SELECT id FROM medicines WHERE LOWER(medicine_name) LIKE LOWER($1) LIMIT 1',
    [`%${name.trim()}%`]
  );
  return partial.rows[0]?.id ?? null;
}
...
app.post('/api/prescription/upload', upload.single('prescription'), async (req, res) => {
  ...
  try {
    ...
    const { consumerId, customer_email } = req.body;

    const resolvedConsumerId = await resolveConsumerId({ consumerId, customer_email });
    if (!resolvedConsumerId) {
      await fs.unlink(req.file.path).catch(() => {});
      return res.status(400).json({
        error: 'consumerId or customer_email required (and must exist)',
        details: { consumerId, customer_email },
      });
    }

    console.log('📤 Prescription uploaded:', req.file.originalname, 'for consumer:', resolvedConsumerId);

    const parsed = await prescriptionParser.parsePrescription(req.file.path, req.file.mimetype);
    ...
    if (!parsed?.success) {
      return res.status(400).json({
        ...
        details: parsed?.error || 'Unknown parse error',
      });
    }
    ...
    try {
      for (const m of medicines) {
        const medName =
          typeof m === 'string' ? m : (m.name || m.medicine || m.medicine_name || '');

        const medicineId = await resolveMedicineId(medName);

        if (!medicineId) {
          unmatched.push({ name: medName });
          continue;
        }
        ...
        const ins = await query(
          ...
          INSERT INTO prescriptions
            (consumer_id, medicine_id, verified, expiry_date, prescription_date)
          VALUES
            ($1, $2, $3, NULL, CURRENT_DATE)
          RETURNING id, consumer_id, medicine_id, verified, expiry_date, prescription_date, created_at
          `,
          [resolvedConsumerId, medicineId, autoVerify]
        );
    ...
    return res.json({
      ...
      savedPrescriptions: inserted,
      unmatchedMedicines: unmatched,
      medicines: parsed.medicines,
      patientInfo: parsed.patientInfo,
      rawText: parsed.rawText,
      note: autoVerify
        ? 'AUTO_VERIFY_PRESCRIPTIONS=true (saved as verified=true)'
        : 'Saved as verified=false (requires verification before refills)',
    });
  } catch (error) {
  console.error('Error handling prescription upload:', error.stack || error);
  return res.status(500).json({ 
    error: error.message,
    type: error.constructor.name  // Shows "ReferenceError", "TypeError" etc.
  });
}
});
